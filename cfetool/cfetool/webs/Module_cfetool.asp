...
function conf2obj(){
	...
	if (dbus["cfetool_mcode"]){
		...
		E("cfetool_mail").href = "mailto:mjy211@gmail.com?subject=CFE国行修改器插件购买&body=订单号：xxx%0d%0a机器码：" +  dbus["cfetool_mcode"];
	...
	if (dbus["cfetool_mcode"] && dbus["cfetool_key"]){
		...
		E("cfetool_a_mail").href = "mailto:mjy211@gmail.com?subject=CFE国行修改器插件购买&body=我的CFE国行修改器插件就激活码遗失了，需要找回！%0d%0a机器码：" +  dbus["cfetool_mcode"];
	}
	if(dbus["cfetool_key"]){
...
function cfeit(action){
	...
	if(action == 2 || action == 3 || action == 4){
		...
		if(action == 4 && ct_key.indexOf('cfetool-') != -1){
			...
			if(ct_key.length == "44" && ct_key.myReplace("-", "").length == "39"){
				...
				msg += '<br/>';
				msg += '<br/>';
				msg += '提示：一个兑换码只能用于一台路由器的【CFE国行修改器】激活；';
				msg += '<br/>';
				msg += '点击立即激活，你将会获得【CFE国行修改器】激活码，同时兑换码将会失效。';
				...
				require(['/res/layer/layer.js'], function(layer) {
					layer.confirm(msg, {
						...
						layer.close(index);
						location.href = "http://" + pay_server + ":" + pay_port + "/pay_cfe_arm.php?paytype=3&uuid=" + ct_key + "&mcode=" + dbus["cfetool_mcode"].replace(/\+/g, "-") + "&router=" + net_address;
						return true;
					});
				});
				E("cfetool_key").value = "";
	...
	if(action == 3){
		if(dbus["cfetool_warn_1"] != "1"){
			if(!confirm('注意：\n检测到首次使用【恢复原始CFE】按钮！\n\n该功能将CFE完全恢复至首次使用CFE国行修改器前(出厂状态)！\n恢复后会自动重启路由器！才能达到效果！\n\n点确定后，将会继续运行该功能，并且此弹出消息以后也不再显示。')){
				return false;
			}else{
...
function get_pay_server() {
	$.ajax({
		url: 'https://armsoft.ddnsto.com/cfetool/config.json.js',
		type: 'GET',
		dataType: 'jsonp',
...
function open_buy() {
	...
	note = "<h2><font color='#FF6600'>【CFE国行修改器】需付费授权，价格为20元人民币。</font></h2>"
	note += "<hr>"
	note += "<h3>建议选择 <font color='#22ab39'>微信支付</font> / <font color='#1678ff'>支付宝</font> 购买，可以即时激活【CFE国行修改器】！</h2>"
	...
	note += "<h4 style='text-align:right'>客服邮箱：<a style='color:#22ab39;' href='mailto:mjy211@gmail.com?subject=【CFE国行修改器】咨询&body=这是邮件的内容'>mjy211@gmail.com</a></h4>"
	require(['/res/layer/layer.js'], function(layer) {
		layer.open({
			type: 0,
			skin: 'layui-layer-lan',
			shade: 0.8,
			title: '请选择【CFE国行修改器】购买方式！',
			time: 0,
			area: '580px',
			offset: '250px',
			btnAlign: 'c',
			...
			btn: ['微信支付', '支付宝', '人工邮件购买'],
			btn1: function() {
				location.href = "http://" + pay_server + ":" + pay_port + "/pay_cfe_arm.php?paytype=1&mcode=" + dbus["cfetool_mcode"].replace(/\+/g, "-") + "&router=" + net_address;
			},
			btn2: function() {
				location.href = "http://" + pay_server + ":" + pay_port + "/pay_cfe_arm.php?paytype=2&mcode=" + dbus["cfetool_mcode"].replace(/\+/g, "-") + "&router=" + net_address;
			},
			btn3: function() {
	...
	require(['/res/layer/layer.js'], function(layer) {
		layer.open({
			...
			content: '<div style="padding: 50px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 300;">\
				<b>CFE国行修改器</b><br><br>\
				CFE国行修改器是一款付费插件，支持hnd/axhnd/axhnd.675x平台的机器，详情：<a style="color:#e7bd16" target="_blank" href="https://github.com/koolshare/armsoft#%E6%9C%BA%E5%9E%8B%E6%94%AF%E6%8C%81"><u>机型支持</u></a><br>\
										...
										<td bgcolor="#4D595D" colspan="3" valign="top">
											...
											<div id="qrcode_show" class="content_status">
												<div style="text-align: center;margin-top:10px">
													<span id="qrtitle" style="font-size:16px;color:#000;">【CFE国行修改器】需付费授权，价格为20元人民币。</span>
												</div>
												...
												<div style="margin-top:5px;margin-left:4%;width:96%;text-align:left;">
													<div id="info0" style="font-size:16px;color:#000;"><i>【CFE国行修改器】人工邮件购买流程：</i></div>
													<div id="info1" style="font-size:12px;color:#000;">&nbsp;&nbsp;&nbsp;&nbsp;1.扫描上方其中一个二维码，付款20元人民币。</div>
													<div id="info2" style="font-size:12px;color:#000;">&nbsp;&nbsp;&nbsp;&nbsp;2.复制下面文本框内容，替换xxx为<a type="button" href="javascript:void(0);" style="cursor: pointer;color:#FF3300;" onclick="pop_help();"><u>支付订单号</u></a>，发送邮件到：<a id="cfetool_mail" style="font-size:12px;color:#CC0000;" href="mailto:mjy211@gmail.com?subject=CFE国行修改器插件购买&body=这是邮件的内容">mjy211@gmail.com</a></div>
											...
											<div id="activated_info" class="content_status" style="height:205px;top:200px">
												<div style="text-align: center;margin-top:15px;margin-bottom:15px">
													<span id="qrtitle_1" style="font-size:16px;color:#000;"><i>你的【CFE国行修改器】已经成功激活！</i></span>
												</div>
											...
											<div class="formfonttitle">CFE国行修改器<lable id="cfetool_version"><lable></div>
											<div style="float:right; width:15px; height:25px;margin-top:-20px">
												<img id="return_btn" onclick="reload_Soft_Center();" align="right" style="cursor:pointer;position:absolute;margin-left:-30px;margin-top:-25px;" title="返回软件中心" src="/images/backprev.png" onMouseOver="this.src='/images/backprevclick.png'" onMouseOut="this.src='/images/backprev.png'"></img>
											</div>
											<div style="margin:10px 0 10px 5px;" class="splitLine"></div>
											<div id="head_note">
												<span>【CFE国行修改器】可以查看路由器CFE内相关信息，并支持非国行机器改国行(收费功能)。</span>
											...
											<div class="SimpleNote" id="message" style="display: none;">
												...
												<li id="msg4">如果不想使用本插件修改CFE，可以参考：<a href="https://www.koolcenter.com/thread/175962" target="_blank">https://www.koolcenter.com/thread/175962</a>，实现人工修改CFE的目的。</li>
												<li id="msg5">由于RT-AC68U机型SDK太老，目前本插件尚不支持这该机型的CFE国行修改。</li>
											</div>
										</td>
									</tr>
								</table>
