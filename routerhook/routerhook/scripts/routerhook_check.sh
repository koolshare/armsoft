...
if [[ "${routerhook_info_temp}" == "1" ]]; then
    ...
    interface_2_temperature=`wl -i ${interface_2} phy_tempsense | awk '{print $1}'` 2>/dev/null
    interface_5_temperature=`wl -i ${interface_5} phy_tempsense | awk '{print $1}'` 2>/dev/null
    ...
    if [ "${interface_2_temperature}" != "" ] || [ "${interface_5_temperature}" != "" ] || [ "${router_cpu_temperature}" != "" ]; then
        ...
        interface_2_temperature_c="null"
        interface_5_temperature_c="null"
        router_cpu_temperature="null"
    fi
    echo '"CPU":'${router_cpu_temperature}',' >> ${routerhook_info_text}
    echo '"5G":'${interface_5_temperature_c}',' >> ${routerhook_info_text}
    echo '"24G":'${interface_2_temperature_c}'' >> ${routerhook_info_text}
    echo '}' >> ${routerhook_info_text}
fi
