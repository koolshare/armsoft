...
install_now(){
	...
	local router_name_get=$(dbus get routerhook_config_name)
	if [ -z "${router_name_get}" ]; then
		dbus set routerhook_config_name="${router_name}"
	fi
	local router_ntp_get=$(dbus get routerhook_config_ntp)
	if [ -z "${router_ntp_get}" ]; then
		dbus set routerhook_config_ntp="ntp1.aliyun.com"
	fi
	local bwlist_en_get=$(dbus get routerhook_dhcp_bwlist_en)
	if [ -z "${bwlist_en_get}" ]; then
		dbus set routerhook_dhcp_bwlist_en="1"
	fi
	local _sckey=$(dbus get routerhook_config_sckey)
	if [ -n "${_sckey}" ]; then
