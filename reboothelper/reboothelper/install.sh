...
install_now(){
	...
	find /koolshare/init.d -name "*Reboothelper*" | xargs rm -rf >/dev/null 2>&1
	find /koolshare/init.d -name "*reboothelper*" | xargs rm -rf >/dev/null 2>&1
	...
	if [ ! -L "/koolshare/init.d/V99${module}.sh" -a -f "/koolshare/scripts/${module}_config.sh" ];then
		ln -sf /koolshare/scripts/${module}_config.sh /koolshare/init.d/V99${module}.sh
	fi
