...
install_now(){
	...
	if [ "${ENABLE}" == "1" -a -f "/koolshare/scripts/kms.sh" ];then
		echo_date "安装前先关闭${TITLE}插件，以保证更新成功！"
		sh /koolshare/scripts/kms.sh stop >/dev/null 2>&1
	fi

	# remove some file first
	find /koolshare/init.d -name "*kms*" | xargs rm -rf >/dev/null 2>&1
	find /koolshare/init.d -name "*Kms*" | xargs rm -rf >/dev/null 2>&1
	...
	# Permissions
	chmod 755 /koolshare/bin/vlmcsd >/dev/null 2>&1
