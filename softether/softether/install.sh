...
install_now(){
	# default value
	local TITLE="SoftEther_VPN_Server"
	local DESCR="VPN全家桶, ver 4.29 build 9680"
	local PLVER=$(cat ${DIR}/version)

	# stop first
	local ENABLE=$(dbus get ${module}_enable)
	if [ "${ENABLE}" == "1" -a -f "/koolshare/softether/softether.sh" ];then
		echo_date "安装前先关闭${TITLE}插件，以保证更新成功！"
		sh /koolshare/softether/softether.sh stop >/dev/null 2>&1
	fi

	# remove some file first
	find /koolshare/init.d -name "*softEther*" | xargs rm -rf >/dev/null 2>&1
	find /koolshare/init.d -name "*SoftEther*" | xargs rm -rf >/dev/null 2>&1
	...
	if [ ! -L "/koolshare/init.d/S98${module}.sh" -a -f "/koolshare/${module}/${module}.sh" ];then
		ln -sf /koolshare/${module}/${module}.sh /koolshare/init.d/S98${module}.sh
	fi
	if [ ! -L "/koolshare/init.d/N98${module}.sh" -a -f "/koolshare/${module}/${module}.sh" ];then
		ln -sf /koolshare/${module}/${module}.sh /koolshare/init.d/N98${module}.sh
	fi
	...
	if [ "${ENABLE}" == "1" -a -f "/koolshare/softether/softether.sh" ];then
		...
		sh /koolshare/softether/softether.sh start
	fi
