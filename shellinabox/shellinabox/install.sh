...
install_now(){
	# default value
	local TITLE="shellinabox工具箱"
	local DESCR="超强的SSH网页客户端~"
	local PLVER=$(cat ${DIR}/version)

	# stop first
	local PID_1=$(pidof shellinaboxd)
	if [ -n "${PID_1}" ];then
	...
	dbus set softcenter_module_${module}_target="target=_blank"

	# re-enable
	local PID_2=$(pidof shellinaboxd)
	if [ -z "${PID_2}" ];then
