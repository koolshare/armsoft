...
install_now(){
	# default value
	local TITLE="易有云"
	local DESCR="易有云 （EasyExplorer） 跨平台文件同步，支持双向同步！"
	local PLVER=$(cat ${DIR}/version)

	# stop first
	local ENABLE=$(dbus get ${module}_enable)
	local PID=$(pidof easy-explorer)
	if [ "${ENABLE}" == "1" ];then
	...
	# Permissions
	chmod 755 /koolshare/bin/easy-explorer >/dev/null 2>&1
	...
	# re-enable
	if [ "${ENABLE}" == "1" -a -f "/koolshare/scripts/easyexplorer_config.sh" ];then
