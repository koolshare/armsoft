...
install_now(){
	...
	local router_name_get=$(dbus get serverchan_config_name)
	if [ -z "${router_name_get}" ]; then
		dbus set serverchan_config_name="${router_name}"
	fi
	local router_ntp_get=$(dbus get serverchan_config_ntp)
	if [ -z "${router_ntp_get}" ]; then
		dbus set serverchan_config_ntp="ntp1.aliyun.com"
	fi
	local bwlist_en_get=$(dbus get serverchan_dhcp_bwlist_en)
	if [ -z "${bwlist_en_get}" ]; then
		dbus set serverchan_dhcp_bwlist_en="1"
	fi
	local _sckey=$(dbus get serverchan_config_sckey)
	if [ -n "${_sckey}" ]; then
